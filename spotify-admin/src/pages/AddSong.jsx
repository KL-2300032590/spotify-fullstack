...
import FetchFromAPI from './FetchFromAPI'; // ⬅️ Import the new component
...
const AddSong = () => {
  ...
  // ⬇️ This function allows FetchFromAPI to set your form values
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      formData.append('duration', duration); // send duration too
  ...
  return loading ? (
    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col items-start gap-8 text-gray-600'>
      ...
      {/* ⬇️ Injected metadata fetch UI component */}
...
const AddSong = () => {
    ...
    const onSubmitHandler = async(e)=>{
        ...
        try {
          ...
          toast.error("response didnt get");
        }
        setLoading(false);
    ...
    const loadAlbumData = async()=>{
      try {
        ...
        if(response.data.success){
          ...
          toast.error("Unable to Album data");
        }
      } catch (error) {
         toast.error("Something went Wrong");
      }
...
=======
     ...
     <div className='flex flex-col gap-2.5'>
      ...
      <select onChange={(e)=>setAlbum(e.target.value)} defaultValue={album} className='bg-transparent outline-green-600 border-gray-400  p-2.5 w-[150px]'>
          <option value="none">none</option>
          {albumData.map((item,index)=>(<option value={item.data}>{item.name}</option>))}
      </select>
     </div>
